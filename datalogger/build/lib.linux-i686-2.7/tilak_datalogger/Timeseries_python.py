...
class Timeseries(object):
    ...
    """
    stat_funcs = {
        "min" : lambda series: min(series),
        "max" : lambda series: max(series),
        "avg" : lambda series: float(sum(series))/len(series),
        "sum" : lambda series: sum(series),
        "std" : lambda series: pstdev(series),
        "median" : lambda series: median(series),
        "count" : lambda series: len(series),
        "first" : lambda series: series[0],
        "latest" : lambda series: series[-1],
        "geomean" : lambda series: geomean(series),
    }
