...
def get_ifXTable(hostname, community):
    """
    wrapper to get IF-MIB::ifXTable nothin special about this one
    uses default version 2c for snmp queries to get also 64 Bit values
    ifXTable includes in contrast to ifTable also High Capacity Counters (64Bit)
    ...
    """
    if_data = get_snmp_table(hostname, "IF-MIB:ifXTable" , community, index=True)
    return if_data
...
def main():
    ...
    for row in centreon.getCentreonHostGroupMembersSnmp(nagios_group):
        ...
        data = get_ifXTable(hostname.split(".")[0], community)
        # print data
